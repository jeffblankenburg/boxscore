...
const STATIC_URLS = (base: string): MetadataRoute.Sitemap => [
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  // Never block a build on a transient DB outage. If Supabase is
  // unreachable at build/regen time, ship the static URLs and let the
  // next revalidate call fill in the dynamic ones. Search crawlers
  // don't need every URL in one hit; they'll re-crawl.
  try {
