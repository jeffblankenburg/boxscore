...
import type { TeammatesPuzzle } from "@/lib/games/teammates/generate";
import { TeamStatCards } from "./StatCards";
import { computeStreaks, type DayResult, type TeammatesStats } from "@/lib/games/teammates/stats";
...
export function TeammatesGame({ puzzle, isAdmin = false }: { puzzle: TeammatesPuzzle; isAdmin?: boolean }) {
  const storeKey = `teammates:${puzzle.date}`;
  ...
  const [stats, setStats] = useState<TeammatesStats | null>(null);
  ...
  useEffect(() => {
    ...
    try {
      for (let i = 0; i < localStorage.length; i++) {
        ...
        if (!k || !k.startsWith("teammates:")) continue;
        const parsed = JSON.parse(localStorage.getItem(k) || "{}");
        if (parsed.status === "won" || parsed.status === "lost") {
          results.push({ date: k.slice("teammates:".length), solved: parsed.status === "won" });
        }
      }
    } catch { /* ignore */ }
...
const SQUARES = ["🟩", "🟨", "🟦", "🟪"];
function copyShare(puzzle: TeammatesPuzzle, guesses: number[][], status: string) {
  const header = `Teammates ${puzzle.date} (${puzzle.difficulty})`;
  const grid = guesses.map((g) => g.map((gi) => SQUARES[gi % 4]).join("")).join("\n");
  const result = status === "won" ? "" : "\nX/4";
  const text = `${header}${result}\n${grid}\nboxscore.email/games/teammates`;
