// Pure streak/stat math for Clubhouse, computed over the player's per-day
...
export type ClubhouseStats = { played: number; wins: number; winPct: number; current: number; max: number };
...
export function computeStreaks(results: DayResult[], today: string): ClubhouseStats {
