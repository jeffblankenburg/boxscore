...
const URL = "http://localhost:3000/games/teammates?date=2026-08-20";
...
async function main() {
  ...
  try {
    ...
    await page.screenshot({ path: resolve("/tmp/teammates-restore.png") as `${string}.png`, fullPage: true });
    console.log("/tmp/teammates-restore.png");
  } finally {
    await browser.close();
  }
}
