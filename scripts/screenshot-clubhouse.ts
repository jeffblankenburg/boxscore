...
const URL = ORIGIN + "/games/clubhouse?date=2026-08-20"; // Astros/Red Sox/Blue Jays/Yankees
  ...
  try {
    ...
    await page.evaluate(() => {
      for (const d of ["2026-08-17", "2026-08-18", "2026-08-19"]) {
        localStorage.setItem("clubhouse:" + d, JSON.stringify({ solvedTeams: [], mistakes: 0, guesses: [], status: "won" }));
      }
    });
    await page.screenshot({ path: resolve("/tmp/clubhouse-initial.png") as `${string}.png`, fullPage: true });
    console.log("/tmp/clubhouse-initial.png");
    ...
    await page.screenshot({ path: resolve("/tmp/clubhouse-flash.png") as `${string}.png`, fullPage: true });
    console.log("/tmp/clubhouse-flash.png");
    ...
    await page.screenshot({ path: resolve("/tmp/clubhouse-won.png") as `${string}.png`, fullPage: true });
    console.log("/tmp/clubhouse-won.png");
    ...
    await new Promise((r) => setTimeout(r, 300));
    await page.screenshot({ path: resolve("/tmp/clubhouse-cards-nyy.png") as `${string}.png`, fullPage: true });
    console.log("/tmp/clubhouse-cards-nyy.png");
  } finally {
    await browser.close();
  }
}
