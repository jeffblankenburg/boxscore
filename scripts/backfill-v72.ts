// Backfill v7.2 across the season so /mlb/predictions can show its
// historical accuracy + Season Picks from day one (and so v7.1↔v7.2 is a
// strict head-to-head), instead of waiting weeks for the live shadow.
//
// Identical mechanics to backfill-v71.ts — regenerate v7.2 from the same
// cached inputs the live engine uses (loadPredictionInputsForDate →
// predictGamesV72), upsert the daily_predictions snapshot, and write the
// graded prediction_results row by COPYING the model-agnostic outcome/odds
// columns from the already-graded v6 row and recomputing only the four
// model-dependent grade fields against v7.2's probabilities. Idempotent.
//
// Run:
//   npx tsx --env-file=.env.local scripts/backfill-v72.ts
//   npx tsx --env-file=.env.local scripts/backfill-v72.ts 2026-06-01 2026-06-30

import { supabaseAdmin } from "../lib/supabase";
import { loadPredictionInputsForDate } from "../lib/sports/mlb/predictions-data";
import { predictGamesV72, V72_MODEL_VERSION } from "../lib/sports/mlb/predictions-v7";
...
async function main() {
  ...
  const end = endArg ?? new Date(Date.now() - 86_400_000).toISOString().slice(0, 10);
  ...
  for (let d = start; d <= end; d = isoNext(d)) {
    ...
    const res = predictGamesV72(inputs);
    ...
    for (const g of res.games) {
      ...
      predUpserts.push({
        sport: "mlb", date: d, game_pk: g.gamePk, model_version: V72_MODEL_VERSION,
